...
export default function MarketplacePage() {
  const { NFTsData, collectionsData } = useContext(DataContext);
  ...
  const nftsToDisplay = NFTsData.slice(
  ...
  const collectionsToDisplay = collectionsData.slice(
    currentPage * itemsPerPage,
    (currentPage + 1) * itemsPerPage,
  );
    ...
    <div className="text-white mt-10 lg:mt-20 font-workSans">
      <div className="px-8 sm:px-32 mb-10 lg:mb-20">
        ...
        <p className="text-lg lg:text-[1.55rem] mt-5 lg:mt-10 font-medium">
          Browse through more than {formatNum(NFTsData.length)} NFTs on the NFT
      ...
      <div className="bg-[#3B3B3B] px-2 lg:px-32 grid md:grid-cols-2 xl:grid-cols-3 justify-items-center pb-10">
      ...
      <ReactPaginate
        ...
        breakLabel={"..."}
        pageCount={Math.ceil(
          (selected === "nfts" ? NFTsData.length : collectionsData.length) /
            itemsPerPage,
        )}
